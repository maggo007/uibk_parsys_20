...
int main(int argc, char **argv) {
  ...
  for (int t = 0; t < T; t++) {
    ...
    if (coord[1] != 0){
      ...
      //printf("send left rank (%d,%d) subsize (%d,%d) start (%d,%d)\n", coord[0], coord[1], subsize[0], subsize[1], substart[0], substart[1]);
      /* MPI_Cart_rank(cart, neighbour, &neighbourrank); */
      /* MPI_Type_create_subarray(DIMENSIONS, arraysize, subsize, substart, MPI_ORDER_C, MPI_DOUBLE, &subarray); */
      /* MPI_Type_commit(&subarray); */
      /* MPI_Send(A, 1, subarray, neighbourrank, 0, MPI_COMM_WORLD); */
    ...
    if (coord[1] < dim[1]-1){
      ...
      subsize[0] = partsize_row;
      subsize[1] = 1;
      //int substart[2] = {start_row, stop_col-1};
      substart[0] = start_row;
      substart[1] = stop_col-1;
      neighbour[0] = coord[0];
      neighbour[1] = coord[1]+1;
      //printf("send right rank (%d,%d) subsize (%d,%d) start (%d,%d)\n", coord[0],coord[1], subsize[0], subsize[1], substart[0], substart[1]);
      /* MPI_Cart_rank(cart, neighbour, &neighbourrank); */
      /* MPI_Type_create_subarray(DIMENSIONS, arraysize, subsize, substart, MPI_ORDER_C, MPI_DOUBLE, &subarray); */
      /* MPI_Type_commit(&subarray); */
      /* MPI_Send(A, 1, subarray, neighbourrank, 0, MPI_COMM_WORLD); */
    ...
    if (coord[0] != 0){
      ...
      neighbour[1] = coord[1];
      //printf("send top rank (%d,%d) subsize (%d,%d) start (%d,%d)\n", coord[0],coord[1], subsize[0], subsize[1], substart[0], substart[1]);
      /* MPI_Cart_rank(cart, neighbour, &neighbourrank); */
      /* MPI_Type_create_subarray(DIMENSIONS, arraysize, subsize, substart, MPI_ORDER_C, MPI_DOUBLE, &subarray); */
      /* MPI_Type_commit(&subarray); */
      /* MPI_Send(A, 1, subarray, neighbourrank, 0, MPI_COMM_WORLD); */
    ...
    if (coord[0] < dim[0]-1){
      ...
      subsize[0] = 1;
      subsize[1] = partsize_col;
      //int substart[2] = {stop_row-1, start_col};
      substart[0] = stop_row-1;
      substart[1] = start_col;
      neighbour[0] = coord[0]+1;
      neighbour[1] = coord[1];
      //printf("send bot rank (%d,%d) subsize (%d,%d) start (%d,%d)\n", coord[0],coord[1], subsize[0], subsize[1], substart[0], substart[1]);
      /* MPI_Cart_rank(cart, neighbour, &neighbourrank); */
      /* MPI_Type_create_subarray(DIMENSIONS, arraysize, subsize, substart, MPI_ORDER_C, MPI_DOUBLE, &subarray); */
      /* MPI_Type_commit(&subarray); */
      /* MPI_Send(A, 1, subarray, neighbourrank, 0, MPI_COMM_WORLD); */
    ...
    if (coord[1] < dim[1]-1){
      ...
      subsize[0] = partsize_row;
      subsize[1] = 1;
      //int substart[2] = {start_row, stop_col};
      substart[0] = start_row;
      substart[1] = stop_col;
      neighbour[0] = coord[0];
      neighbour[1] = coord[1]+1;
      //printf("get from right rank (%d,%d) subsize (%d,%d) start (%d,%d)\n", coord[0],coord[1], subsize[0], subsize[1], substart[0], substart[1]);
      /* MPI_Cart_rank(cart, neighbour, &neighbourrank); */
      /* MPI_Type_create_subarray(DIMENSIONS, arraysize, subsize, substart, MPI_ORDER_C, MPI_DOUBLE, &subarray); */
      /* MPI_Type_commit(&subarray); */
      /* MPI_Recv(A, 1, subarray, neighbourrank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); */
      /* MPI_Type_free(&subarray); */
    ...
    if (coord[1] != 0){
      ...
      subsize[0] = partsize_row;
      subsize[1] = 1;
      //int substart[2] = {start_row, start_col-1};
      substart[0] = start_row;
      substart[1] = start_col-1;
      neighbour[0] = coord[0];
      neighbour[1] = coord[1]-1;
      //printf("get from left rank (%d,%d) subsize (%d,%d) start (%d,%d)\n", coord[0],coord[1], subsize[0], subsize[1], substart[0], substart[1]);
      /* MPI_Cart_rank(cart, neighbour, &neighbourrank); */
      /* MPI_Type_create_subarray(DIMENSIONS, arraysize, subsize, substart, MPI_ORDER_C, MPI_DOUBLE, &subarray); */
      /* MPI_Type_commit(&subarray); */
      /* MPI_Recv(A, 1, subarray, neighbourrank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); */
    ...
    if (coord[0] != 0){
      ...
      subsize[0] = 1;
      subsize[1] = partsize_col;
      //int substart[2] = {start_row-1, start_col};
      substart[0] = start_row-1;
      substart[1] = start_col;
      neighbour[0] = coord[0]-1;
      neighbour[1] = coord[1];
      //printf("get from top rank (%d,%d) subsize (%d,%d) start (%d,%d)\n", coord[0],coord[1], subsize[0], subsize[1], substart[0], substart[1]);
      /* MPI_Cart_rank(cart, neighbour, &neighbourrank); */
      /* MPI_Type_create_subarray(DIMENSIONS, arraysize, subsize, substart, MPI_ORDER_C, MPI_DOUBLE, &subarray); */
      /* MPI_Type_commit(&subarray); */
      /* MPI_Recv(A, 1, subarray, neighbourrank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); */
    ...
    if (coord[0] < dim[0]-1){
      ...
      subsize[0] = 1;
      subsize[1] = partsize_col;
      //int substart[2] = {stop_row, start_col};
      substart[0] = stop_row;
      substart[1] = start_col;
      neighbour[0] = coord[0]+1;
      neighbour[1] = coord[1];
      //printf("get from bot rank (%d,%d) subsize (%d,%d) start (%d,%d)\n", coord[0],coord[1], subsize[0], subsize[1], substart[0], substart[1]);
      /* MPI_Cart_rank(cart, neighbour, &neighbourrank); */
      /* MPI_Type_create_subarray(DIMENSIONS, arraysize, subsize, substart, MPI_ORDER_C, MPI_DOUBLE, &subarray); */
      /* MPI_Type_commit(&subarray); */
      /* MPI_Recv(A, 1, subarray, neighbourrank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); */
