                  ...
                  double a[4], int k)
{
  ...
  /* int i3, i2, i1; */
  double u1prev, u2prev, u1curr, u2curr, u1next, u2next;
  if (timeron) timer_start(T_resid);
#pragma omp parallel for collapse(2) schedule(static,64)
  for (int i3 = 1; i3 < n3-1; i3++) {
    for (int i2 = 1; i2 < n2-1; i2++) {

      u1prev = u[i3][i2-1][0] + u[i3][i2+1][0] +u[i3-1][i2][0] + u[i3+1][i2][0];

      u2prev = u[i3-1][i2-1][0] + u[i3-1][i2+1][0] + u[i3+1][i2-1][0] + u[i3+1][i2+1][0];

      u1curr = u[i3][i2-1][1] + u[i3][i2+1][1] +u[i3-1][i2][1] + u[i3+1][i2][1];
      u2curr = u[i3-1][i2-1][1] + u[i3-1][i2+1][1] + u[i3+1][i2-1][1] + u[i3+1][i2+1][1];
#pragma omp simd
      for (int i1 = 1; i1 < n1-1; i1++) {

        u1next = u[i3][i2-1][i1+1] + u[i3][i2+1][i1+1] +u[i3-1][i2][i1+1] + u[i3+1][i2][i1+1];
        u2next = u[i3-1][i2-1][i1+1] + u[i3-1][i2+1][i1+1] + u[i3+1][i2-1][i1+1] + u[i3+1][i2+1][i1+1];
        
        r[i3][i2][i1] = v[i3][i2][i1] - a[0] * u[i3][i2][i1]
        //-------------------------------------------------------------------
        //  Assume a[1] = 0      (Enable 2 lines below if a[1] not= 0)
        //-------------------------------------------------------------------
        // r[i3][i2][i1] -= a[1] * ( u[i3][i2][i1-1] + u[i3][i2][i1+1]
        //                           + u1[i1] );
        //-------------------------------------------------------------------
         - a[2] * ( u2curr + u1prev + u1next )
         - a[3] * ( u2prev + u2next );
