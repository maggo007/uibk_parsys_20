...
int main(int argc, char **argv) {
  ...
  int shm_fdA, shm_fdB; 
  void* ptr_memA, *ptr_memB;

  if (rank == 0){
    shm_fdA = shm_open("shmA", O_CREAT | O_RDWR, 0666);
    shm_fdB = shm_open("shmB", O_CREAT | O_RDWR, 0666);
    ftruncate(shm_fdA, sizeof(float)*N*M);
    ftruncate(shm_fdB, sizeof(float)*N*M);
  }
